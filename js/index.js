...
function auto(){
    ...
    $('.bannerpic').eq(index).fadeIn(300).siblings().fadeOut(300);
};
//点击底部按钮图片切换
$(".banner-btn li").click(function() {
    var index = $(this).index(); //获取索引 图片索引与按钮的索引是一一对应的
    // console.log(index);
    $(".bannerpic").eq(index).fadeIn(300).siblings().fadeOut(300); // siblings  找到 兄弟节点(不包括自己）
});
//上一张、下一张切换
$("#left").click(function() {
    index--;
    ...
    // console.log(index);
     $(".bannerpic").eq(index).fadeIn(300).siblings().fadeOut(300); // siblings  找到 兄弟节点(不包括自己）必须要写
...
$(window).scroll(function(){goTopHide()});
